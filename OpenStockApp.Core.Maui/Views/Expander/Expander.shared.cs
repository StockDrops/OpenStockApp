using OpenStockApp.Core.Maui.Views.Base;
using System;
...
namespace OpenStockApp.Core.Maui.Views
{
	...
	public class Expander : BaseTemplatedView<StackLayout>
	{
		...
		protected override void OnSizeAllocated(double width, double height)
		{
			...
			if ((Math.Abs(width - previousSize.Width) >= double.Epsilon && Direction.IsVertical()) ||
				(Math.Abs(height - previousSize.Height) >= double.Epsilon && !Direction.IsVertical()))
				ForceUpdateSize();
		...
		void InvokeAnimation(double startSize, double endSize, bool shouldIgnoreAnimation)
		{
			...
			if (lastVisibleSize > 0)
				length = (uint)(length * (Math.Abs(endSize - startSize) / lastVisibleSize));
		...
		void OnTapped() => tappedEventManager.HandleEvent(this, EventArgs.Empty, nameof(Tapped));
	}
}
